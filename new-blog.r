REBOL  [
    ...
    Purpose: {
        ...
        Modified by Dirk Weyand to work smoothly with Serve-It!,
        the smart server-engine based on REBOL/View.
    }
...
; 12. Works now smoothly with Serve-It! v2.3.2! 
;     All modifications of Carl´s Blogger script are marked with ; (TGD) ;
;     Download Serve-It! @ http://www.TGD-Consulting.de/Download.html#reblets 
...
html-template: {
    ...
    <a href=http://www.rebol.com>Blogger by REBOL</a> , <a href=http://www.TGD-Consulting.DE/Download.html#reblets>Serve-It! by TGD-Consulting</a></td></tr>
...
read-cgi: func [
    ...
    switch system/options/cgi/request-method [
        "POST" [data: system/script/args] ;  (TGD) ; Serve-It! liefert POST-Data per system/script/args an das CGI-script
...
; (TGD) ;    cgi: construct/with decode-cgi read-cgi cgi-obj
    cgi: make cgi-obj decode-cgi read-cgi 
    if all [not abort found? cgi/view] [show-blog cgi/view]      ; (TGD) ;
    ...
    if all [not abort  found? cgi/back][backup-blogs cgi/back]   ; (TGD) ;
    if all [not abort found? cgi/rebuild] [rebuild-blogs]        ; (TGD) ;
    if not abort [show-main ]                                    ; (TGD) ;
]

if not exists? blog-dir [make-dir/deep blog-dir]                   ; (TGD) ;
if all [blog-root not exists? blog-root] [make-dir/deep blog-root] ; (TGD) ;
