REBOL [
    Title:   "PHP-colorizer"
    Date:    20-Jan-2009/2:15:01+1:00
    Name:    "Rebol's PHP Code colorizer"
    Version: 1.1.0
    File:    %php-colorizer.r
    Author:  "David 'Oldes' Oliva"
    Email:   oliva.david@seznam.cz
    Home:    http://box.lebeda.ws/~hmm/
    Usage:   [
		php-colorizer/footer-final: {
		<script src="http://www.google-analytics.com/urchin.js" type="text/javascript"></script>
		<script type="text/javascript">_uacct = "UA-1886645-1";urchinTracker();</script>}
		
		php-colorize/save/title
		    %test-php.txt
		    %test-php.html
		    "Rebol's PHP Code Colorizer Test"
	]
    Purpose: none
    Comment: {
    	To change colors, download this CSS style: http://box.lebeda.ws/~hmm/css/cc-php.css
    	modify it and use it (change php-colorizer/css-file to your new version)
    }
    History: [
    	1.1.0 20-Jan-2009 {Using hash! table instead of parse rules to get word's group classes}
    	1.0.0 27-Nov-2007 {First version (based on %code-colorizer.r script)}
	]
    Language: none
    Type:     none
    Content:  none
...
;## Default settings
    ...
    out: tmp: x: pos: new:  none
	index-html:    make string! 1000
	css-file:      http://box.lebeda.ws/~hmm/css/cc-php.css
...
;** These charsets are used in string based parse in Colorize function
	...
	ch_numbers1-9:    charset "123456789"
	ch_numbers0-7:    charset "01234567"
	...
	ch_sign:          charset "-+"
	ch_hexadecimal:   charset ["123456789" #"a" - #"f" #"A" - #"F"]
	ch_php_string1:   complement charset "\'"
	ch_php_string2:   complement charset {\"}
	ch_heredoc:       union ch_alphanum charset "_"
	ch_php_wordStart: union ch_alpha    charset [#"_" #"^(7f)" - #"^(ff)"]
	ch_php_word:      union ch_alphanum charset [#"_" #"^(7f)" - #"^(ff)"]
	ch_symbols:       charset "!-+=*/|.,;^^&%~?:<>@"
	ch_php_bracketsOpen: charset "[({"
	ch_php_bracketsEnd:  charset "])}"
	ch_php_brackets: union ch_php_bracketsOpen ch_php_bracketsEnd
	ch_php_exec:     complement charset "`"

;## Rules used for parsing

	rl_php_comment-single: [
		some [
			["//" | "#"] [any ch_space any ch_tonewline] ;any ch_newlines
		] new:
	]
	rl_php_comment-multiline: [
		some [
			"/*" thru "*/" any ch_newlines
		] new: 
	]
	
	rl_php_decimal: [[some ch_numbers1-9 any ch_numbers | #"0"] new:]
	rl_hexadecimal: [#"0" [#"x" | #"X"] some ch_hexadecimal new:]
	rl_octal:       [#"0" some ch_numbers0-7 new:]
	
	rl_float: [
		 	 some ch_numbers #"." any ch_numbers new:
			| #"." some ch_numbers new: 
	]
	rl_float_exp: [
		[rl_float | some ch_numbers] [#"e" | #"E"] opt ch_sign some ch_numbers
		new: 
	]
	rl_php_string: [
		opt #"b" [
	...
	rl_php_heredoc: [
		opt #"b"
		"<<<" copy x [ch_alpha any ch_heredoc] new: (
			emit 'br1
			either new: find pos join x ";" [
				emit 'st0
				new: find/tail pos ";"
				emit 'br1
			][	 ];missing heredoc end
	...
	rl_php_exec: [#"`" any ch_php_exec #"`" new:]
	rl_php_name: [ch_php_wordStart any ch_php_word new:]
	rl_php_variable: [#"$" opt rl_php_name new:]
	rl_php_setvariable: [rl_php_variable any ch_spaces #"=" new:]
	rl_php_symbols: [some ch_symbols new:]
	...
	rl_php_superglobals: [
		["$GLOBALS"|"$_SERVER"|"$_GET"|"$_POST"|"$_COOKIE"|"$_FILES"|"$_ENV"|"$_REQUEST"|"$_SESSION"] new:
	]
	rl_php_logic: [
		["true"|"false"] new:
	]
	
;## Main PHP's word groups
	...
	rl_php_words2: [
		...
		"false" "true" "var" "default"  
		"function" "class" "new" "public" "private" "interface" "extends"  
	...
	rl_php_words3: [
            ...
            "yp_cat" "yp_all" "xml_set_unparsed_entity_decl_handler" "xml_set_start_namespace_decl_handler"
            "xml_set_processing_instruction_handler" "xml_set_object" "xml_set_notation_decl_handler"
            "xml_set_external_entity_ref_handler" "xml_set_end_namespace_decl_handler" "xml_set_element_handler"
            "xml_set_default_handler" "xml_set_character_data_handler" "xml_parser_set_option" "xml_parser_get_option"
            "xml_parser_free" "xml_parser_create_ns" "xml_parser_create" "xml_parse_into_struct" "xml_parse"
            "xml_get_error_code" "xml_get_current_line_number" "xml_get_current_column_number" "xml_get_current_byte_index" "xml_error_string" 
            ...
            "apache_child_terminate" "aggregation_info" "aggregate_properties_by_regexp" "aggregate_properties_by_list"
            "aggregate_properties" "aggregate_methods_by_regexp" "aggregate_methods_by_list" "aggregate_methods"
            "aggregate" "addslashes" "addcslashes" "acosh" "acos" "abs" "_" "echo" "print" "global" "static" "exit" "array" "empty" 
...
;## escape-html
	ch_notEscapeHtml: complement charset "&<>"
	escape-html: func[data [string! binary!] /local p][
		parse/all data [ any [
			any ch_notEscapeHtml
			p: [
				  #"<" (p: change/part p "&lt;"  1) :p
				| #">" (p: change/part p "&gt;"  1) :p
				| #"&" (p: change/part p "&amp;" 1) :p
...
;## emit
	emit: func[cl /local l][
		;probe copy/part pos 50
		;probe copy/part new 50
		pos: insert pos l: ajoin [{<i class=} cl {>}]
		new: change/part pos escape-html copy/part pos (new: skip new length? l) new
		pos: insert new "</i>"
		;probe copy/part head pos 50
		pos
	]
...
;## php-colorize
	set 'php-colorize func[source [any-string! file! url!] /save outfile /title ttl /local source-file out][
		out: either any [file? source url? source][
			source-file: source
			read source
		][	source ]
		...
		loop 1 [ ;<-- to be able break parsing
			parse/all out: detab out [
				any [
					pos:
					  rl_php_comment-single    (emit either pos/1 = #"#" ['co1]['co2]) :pos
					| rl_php_comment-multiline (emit 'co3) :pos
					| opt ch_sign [ 
						 [rl_float_exp | rl_float] (emit 'nu3) :pos
						| rl_hexadecimal       (emit 'nu1) :pos
						| rl_octal             (emit 'nu2) :pos
						| rl_php_decimal       (emit 'nu0) :pos
					]
					| rl_php_string            (emit 'st0) :pos
					| rl_php_heredoc
					| rl_php_superglobals      (emit 'gl) :pos
					| rl_php_setvariable       (emit 'sw) :pos
					| rl_php_variable          (emit 'lw) :pos
					| rl_php_typeCasting       (emit 'tc) :pos
					| rl_php_logic             (emit 'kw5) :pos
					| rl_php_name    
			          (
			          	emit any [
			          		select word-classes copy/part pos new
							'kw4
						]
					) :pos
					| rl_php_symbols           (emit 'kw6) :pos
					| ch_php_brackets new:     (emit 'br0) :pos
					| rl_php_exec              (emit 'st1) :pos
					| some ch_spaces ;(emit x)
					| copy x to end (unless x [break])
					()
				]
				(
					if level-string > 0 [print "!!! Invalid string!"]
					if break-on-error? [break]
				)
			]
		]
		unless empty? index-html [append index-html "</ol>"]
	    if save [
			write/binary outfile rejoin [
				{<html><head>}
				{<LINK rel="stylesheet" href="} css-file {"/>}
				{<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=windows-1250">}
				{<title>} any [ttl rejoin ["a " language " code"]] {</title>}
				{</head>^/}
				{<body bgcolor="#ffffff">}
				{<div class=header>}
				...
				{ by <a href="http://box.lebeda.ws/~hmm/rebol/php-colorizer.r">%php-colorizer.r</a> Rebol script</div>}
				any [footer-final ""]
				{</body></html>}
			]
		]
